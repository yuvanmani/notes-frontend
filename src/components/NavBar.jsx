            ...
            <div>
                <nav className="bg-gray-800 md:bg-gradient-to-bl from-gray-500 to-gray-800 text-white shadow-md fixed top-0 left-0 w-full z-50">
                    <div className="container mx-auto px-6 py-4 flex xl:justify-evenly justify-around items-center">
                        <p className="text-2xl text-white md:text-2xl font-semibold hover:text-orange-400"><span className="text-3xl">M</span><span
                        ...
                        <ul className="hidden md:flex space-x-6">
                            {
                                getNavigation(user).map((item) => (
                                    <li key={item.name} className="font-semibold text-md hover:text-orange-400 lg:text-lg lg:font-normal">
                            ...
                            {
                                user && (
                                    <p className="lg:pt-0.5 hover:text-orange-400">{user.email}</p>
                                )
                            }
                        </ul>
                        <button className="md:hidden text-3xl text-white hover:text-orange-400" onClick={handleMenu}>&#9776;</button>
                    </div>
                    {
                        isMenuOpen &&
                        <ul className="bg-gradient-to-bl from-gray-400 to-gray-800 text-white shadow-md md:hidden absolute top-16 left-150 w-full p-4">
                            {
                                getNavigation(user).map((item) => (
                                    <li key={item.name}
                                        className="text-md font-semibold hover:text-orange-400 mb-2" onClick={closeMenu}>
                                        <Link to={item.href}>{item.name}</Link>
                                    </li>
                                ))
                            }
                            {
                                user && (
                                    <p className="hover:text-orange-400">{user.email}</p>
                                )
                            }
                        </ul>
                    }
